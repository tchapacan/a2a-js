...
import { restTransformMiddleware } from './utils.js';
 ...
 *
 * Catches syntax errors from express.json() and returns properly formatted
 * REST error responses with appropriate HTTP status codes.
 *
...
export function httpRestHandler(options: HttpRestHandlerOptions): RequestHandler {
  ...
  // Parse JSON bodies for REST endpoints, with error handling
  router.use(express.json(), restErrorHandler);

  // Transform REST requests and responses (snake_case ↔ camelCase)
  router.use(restTransformMiddleware());
   ...
   */
  const extractAction = (path: string, pattern: RegExp, groupIndex: number = 1): ParsedAction => {
